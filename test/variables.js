...
describe("Liquid.Variable", function() {
  ...
  it("parses filters with date formatting parameter", function() {
    var v = new Liquid.Variable(" '2006-06-06' | date: \"%m/%d/%Y\" ");
  ...
  describe("with multiple arguments", function() {
    it("parses ", function() {
      var v = new Liquid.Variable("'typo' | link_to: 'Typo', true");
      expect("'typo'").to.equal(v.name);
      return expect([["link_to", ["'Typo'", "true"]]]).to.deep.equal(v.filters);
    });

    it("parses", function() {
      var v = new Liquid.Variable("'typo' | link_to: 'Typo', false");
      expect("'typo'").to.equal(v.name);
      return expect([["link_to", ["'Typo'", "false"]]]).to.deep.equal(v.filters);
    });

    it("parses", function() {
      var v = new Liquid.Variable("'foo' | repeat: 3");
      expect("'foo'").to.equal(v.name);
      return expect([["repeat", ["3"]]]).to.deep.equal(v.filters);
    });

    it("parses", function() {
      var v = new Liquid.Variable("'foo' | repeat: 3, 3");
      expect("'foo'").to.equal(v.name);
      return expect([["repeat", ["3", "3"]]]).to.deep.equal(v.filters);
    });

    it("parses", function() {
      var v = new Liquid.Variable("'foo' | repeat: 3, 3, 3");
      expect("'foo'").to.equal(v.name);
      return expect([["repeat", ["3", "3", "3"]]]).to.deep.equal(v.filters);
    });

    return it(
      "parses when a string-argument contains an argument-separator",
      function() {
        var v = new Liquid.Variable(" hello | things: \"%Y, okay?\", 'the other one'");
        expect("hello").to.equal(v.name);
  ...
  it("allows ranges", function() {
    return renderTest("1-2-3", "{{ (1..3) | join:\"-\" }}");
  });
  ...
  context("with filter", function() {
    it("renders", function() {
      var MoneyFilter = {
        ...
        money_with_underscore: function(input) {
          return util.format(" $%d ", input);
        }
      };

      var context = new Liquid.Context();
      context.set("var", 1000);
      context.registerFilters(MoneyFilter);
      var variable = new Liquid.Variable("var | money");

      return variable.render(context).then(function(result) {
        return expect(result).to.equal(" $1000 ");
      });
    });

    it("renders empty string", function() {
      return renderTest("", "{{ test | append: \"\" }}", {});
    });

    return it("renders on unknown filter", function() {
      return renderTest(
        /filter 'doesNotExist' in ' 1 \| doesNotExist ' could not be found/,
        "{{ 1 | doesNotExist }}",
        {},
        false
      );
